...
export class ToolbarMainComponent implements OnInit, OnDestroy
{
    ...
    public ngAfterContentInit(){
        $(".online-friends-box").click(function(){
            if($(this).css('height') == "70px"){
    
                $(this).css({'height': '', 'bottom': '' });
            }else{
                $(".online-friends-box").css({'height': '', 'bottom': '' })
                $(this).css({'height': '70px', 'bottom': '34px' });
            }
        })
  ...
  /*  buyut(e: any)
    {   const classList = e.currentTarget.classList;
        const classess = e.currentTarget.className;

       
        if (e.currentTarget.id === "friends") { classess.includes('friends-box-acik') ? classList.remove('friends-box-acik') : classList.add('friends-box-acik'); }
    ...
    public ngOnInit(): void
    {
        ...
        /*  $(function(){
             ...
             setTimeout( $(".online-friends-box").click(function(){
              if($(this).css('height') == "70px"){
                  $(this).css({'height': '', 'bottom': '' });
              }else{
                  $(".online-friends-box").css({'height': '', 'bottom': '' })
        ...
        const slider = document.querySelector(".bottombar-friends-list");

        $(".friends-right-box").click(function ()
        {
            console.log("xx")
            slider.scrollLeft += 130;
        });

        $(".friends-left-box").click(function ()
        {
            console.log("xx")
            slider.scrollLeft -= 130;
        });
        ...
        $(".bri-slide-box").click(function ()
        {
            xx++;
            if (xx % 2 == 0)
            {
                $(this).css({ 'height': '172px', 'top': '0px', 'background-image': 'url("./images/bottombar/bri-close.png")' });
                $(".bri-room-options").css('display', 'block')
                $(".bri-room-name_and_owner-box").css('display', 'block')
            } else
            {
                $(this).css({ 'height': '108px', 'top': '0px', 'background-image': 'url("./images/bottombar/bri-open.png")' });
                $(".bri-room-options").css('display', 'none')
                $(".bri-room-name_and_owner-box").css('display', 'none')
            }

        });

        $(".bottom-right-ufalt").click(function ()
        {
            xx++;
            if (xx % 2 == 0)
            {
                $(this).css({ 'background-image': 'url("./images/bottombar/sag-kapat.png")' });
                $(".bottombar-friends-list-box").css('display', 'block')
            } else
            {
                $(this).css({ 'background-image': 'url("./images/bottombar/sag-ac.png")' });
                $(".bottombar-friends-list-box").css('display', 'none')
            }
    ...
    public get friends(): MessengerFriend[]
    {
        ...
        return friends.filter(friend => { return friend.online; });
    }
}